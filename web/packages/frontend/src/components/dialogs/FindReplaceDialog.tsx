...
const FindReplaceDialog: React.FC = () => {
  ...
  const doReplace = useCallback(async (all: boolean) => {
    if (!findText || matches.length === 0 || replacing) return;
    setReplacing(true);
  ...
  }, [findText, replaceText, matches, matchCase, tasks, batchUpdateTasks, showSnackbar, doFind, replacing]);
  ...
  return (
    <Dialog open={open} onClose={closeDialog} maxWidth="sm" fullWidth>
      ...
      <DialogActions>
        ...
        <Button onClick={() => doReplace(false)} disabled={matches.length === 0 || replacing}>
          {replacing ? 'Replacing...' : 'Replace'}
        </Button>
        <Button onClick={() => doReplace(true)} disabled={matches.length === 0 || replacing}>
