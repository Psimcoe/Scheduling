...
const GanttBar: React.FC<GanttBarProps> = ({
  ...
  // Resolve bar color from stored bar styles
  const getStyleColor = (showFor: string) =>
    barStyles.find((s) => s.showFor === showFor)?.barColor;

  const barColor =
    showCriticalPath && task.isCritical
      ? (getStyleColor('critical') ?? '#D32F2F')
      : isSummary
        ? (getStyleColor('summary') ?? '#333333')
        : isSelected
          ? '#1B6B3A'
          : (getStyleColor('task') ?? '#4694E4');
  ...
  const handleDragStart = useCallback(
    (e: React.MouseEvent, type: 'move' | 'resize') => {
      if (isSummary || dragRef.current) return; // Don't drag summary tasks; prevent concurrent drags
      e.preventDefault();
      e.stopPropagation();
