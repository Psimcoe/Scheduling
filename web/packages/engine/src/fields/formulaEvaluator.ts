...
function tokenize(expr: string): Token[] {
  ...
  while (i < expr.length) {
    ...
    if (ch === '[') {
      let field = '';
      i++;
      while (i < expr.length && expr[i] !== ']') field += expr[i++];
      i++; // skip ]
...
function resolveField(task: Record<string, unknown>, fieldName: string): unknown {
  const mapped = FIELD_MAP[fieldName] ?? fieldName;
  // Try exact property name, then camelCase version
  if (mapped in task) return task[mapped];
  const camel = mapped.charAt(0).toLowerCase() + mapped.slice(1);
  if (camel in task) return task[camel];
  return 0;
}
