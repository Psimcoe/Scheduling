...
describe('stratusSyncService', () => {
  ...
  it('builds sync-to-prefab preview rows for changed package reference tasks only', () => {
    ...
    expect(rows[0]?.changes).toEqual([
      { field: 'start', from: '2026-03-01T00:00:00.000Z', to: '2026-03-05T00:00:00.000Z' },
      { field: 'finish', from: '2026-03-02T00:00:00.000Z', to: '2026-03-06T00:00:00.000Z' },
      { field: 'deadline', from: '2026-03-03T00:00:00.000Z', to: '2026-03-07T00:00:00.000Z' },
    ]);
  });
  ...
  it('builds refresh-from-prefab preview rows for changed reference tasks only', () => {
    ...
    expect(rows[0]?.prefabTaskId).toBe('prefab-package');
    expect(rows[0]?.changes).toEqual([
      { field: 'start', from: '2026-03-05T00:00:00.000Z', to: '2026-03-01T00:00:00.000Z' },
      { field: 'finish', from: '2026-03-06T00:00:00.000Z', to: '2026-03-02T00:00:00.000Z' },
      { field: 'deadline', from: '2026-03-07T00:00:00.000Z', to: '2026-03-03T00:00:00.000Z' },
    ]);
  });
});
