...
export function getRequestedStratusFieldKeys(
  config: Pick<
    StratusConfig,
    | "taskNameField"
    | "durationDaysField"
  ...
  const keys = new Set<string>(BASE_STRATUS_FIELD_KEYS);
  ...
  keys.add(STRATUS_TASK_NAME_FIELD_NAME);
  keys.add(STRATUS_DURATION_DAYS_FIELD_NAME);
  keys.add(STRATUS_DURATION_HOURS_FIELD_NAME);
  keys.add(STRATUS_START_DATE_FIELD_NAME);
  keys.add(STRATUS_FINISH_DATE_FIELD_NAME);
  keys.add(STRATUS_DEADLINE_FIELD_NAME);
...
export function resolveFieldIdsFromDefinitions(
  ...
  const startFieldName =
    normalizeOptionalString(overrides.startDateField) ??
    STRATUS_START_DATE_FIELD_NAME;
  const finishFieldName =
    normalizeOptionalString(overrides.finishDateField) ??
    STRATUS_FINISH_DATE_FIELD_NAME;
  const deadlineFieldName =
    normalizeOptionalString(overrides.deadlineField) ??
    STRATUS_DEADLINE_FIELD_NAME;

  const manualStart = normalizeOptionalString(
    overrides.startDateFieldIdOverride,
  );
  const manualFinish = normalizeOptionalString(
    overrides.finishDateFieldIdOverride,
  );
  const manualDeadline = normalizeOptionalString(
    overrides.deadlineFieldIdOverride,
  );
  ...
  const cachedDeadline = normalizeOptionalString(cached.cachedDeadlineFieldId);

  const startMatch = findFieldDefinitionMatch(
    fields,
    getEquivalentFieldNames(startFieldName),
  );
  const finishMatch = findFieldDefinitionMatch(
    fields,
    getEquivalentFieldNames(finishFieldName),
  );
  const deadlineUsesProperty =
    deadlineFieldName.toLowerCase() ===
    STRATUS_DEADLINE_FIELD_NAME.toLowerCase();
  const deadlineMatch = deadlineUsesProperty
    ? { matchId: null, ambiguous: false }
    : findFieldDefinitionMatch(
        fields,
        getEquivalentFieldNames(deadlineFieldName),
      );

  const startFieldId = manualStart ?? cachedStart ?? startMatch.matchId;
  const finishFieldId = manualFinish ?? cachedFinish ?? finishMatch.matchId;
  const deadlineFieldId = deadlineUsesProperty
    ? null
    : (manualDeadline ?? cachedDeadline ?? deadlineMatch.matchId);

  const problems: string[] = [];
  if (!startFieldId) {
    problems.push(`Unable to resolve ${startFieldName}`);
  } else if (startMatch.ambiguous && !manualStart) {
    problems.push(`${startFieldName} matched multiple company fields`);
  }
  if (!finishFieldId) {
    problems.push(`Unable to resolve ${finishFieldName}`);
  } else if (finishMatch.ambiguous && !manualFinish) {
    problems.push(`${finishFieldName} matched multiple company fields`);
  }
  if (!deadlineUsesProperty && !deadlineFieldId) {
    problems.push(`Unable to resolve ${deadlineFieldName}`);
  } else if (
    !deadlineUsesProperty &&
    deadlineMatch.ambiguous &&
    !manualDeadline
  ) {
    problems.push(`${deadlineFieldName} matched multiple company fields`);
  }
  ...
  return {
    ...
    finishFieldId,
    deadlineFieldId,
    deadlineMode: deadlineUsesProperty ? "property" : "field",
    canPush: Boolean(
      startFieldId &&
      finishFieldId &&
      (deadlineUsesProperty || deadlineFieldId) &&
      problems.length === 0,
    ),
...
export function normalizeStratusPackage(
  ...
  const normalizedFields: Record<string, string | null> = {
    "STRATUS.Field.Project Number": getFieldValue(fieldMap, [
      "STRATUS.Field.Project Number",
    ]),
    "STRATUS.Field.Cost_Code_Number": getFieldValue(fieldMap, [
      "STRATUS.Field.Cost_Code_Number",
    ]),
    ...
    "STRATUS.Field.Project Number Override": getFieldValue(fieldMap, [
      "STRATUS.Field.Project Number Override",
    ]),
    "STRATUS.Field.Project Name Override": getFieldValue(fieldMap, [
      "STRATUS.Field.Project Name Override",
    ]),
    "STRATUS.Field.SMC_Location": getFieldValue(fieldMap, [
      "STRATUS.Field.SMC_Location",
    ]),
    "STRATUS.Field.Detailer": getFieldValue(fieldMap, [
      "STRATUS.Field.Detailer",
    ]),
    "STRATUS.Field.Foreman Contact": getFieldValue(fieldMap, [
      "STRATUS.Field.Foreman Contact",
    ]),
    "STRATUS.Package.Number": packageNumber,
    "STRATUS.Package.Name": firstNonEmptyString(
      getString(rawPackage, "name"),
      packageName,
    ),
    "STRATUS.Field.Cost_Code_Category": getFieldValue(fieldMap, [
      "STRATUS.Field.Cost_Code_Category",
    ]),
    "STRATUS.Package.CategoryAbbreviation": getFieldValue(fieldMap, [
      "STRATUS.Package.CategoryAbbreviation",
    ]),
    ...
    "STRATUS.Field.Assembly_Qty_Editable": getFieldValue(fieldMap, [
      "STRATUS.Field.Assembly_Qty_Editable",
    ]),
    "Detail Due By Date": parseDateToIso(
      getFieldValue(fieldMap, ["Detail Due By Date"]),
    ),
    "STRATUS.Field.SMC_CREW SIZE": getFieldValue(fieldMap, [
      "STRATUS.Field.SMC_CREW SIZE",
    ]),
    ...
    "STRATUS.Package.RequiredDT": firstNonEmptyString(
      ...
      parseDateToIso(
        getFieldValue(fieldMap, ["STRATUS.Package.RequiredDT", "Required"]),
      ),
    ),
    "STRATUS.Field.SMARTSHEET TO STRATUS OVERRIDE": getFieldValue(fieldMap, [
      "STRATUS.Field.SMARTSHEET TO STRATUS OVERRIDE",
    ]),
    "STRATUS.Package.TrackingStatus": trackingStatusName ?? statusName,
    "STRATUS.Package.Notes": getFieldValue(fieldMap, [
      "STRATUS.Package.Notes",
      "Notes",
    ]),
    "Created By": firstNonEmptyString(
      getString(rawPackage, "createdBy"),
      getFieldValue(fieldMap, ["Created By"]),
    ),
    "Date Created": firstNonEmptyString(
      parseDateToIso(getString(rawPackage, "createdDT")),
      getFieldValue(fieldMap, ["Date Created"]),
    ),
    "Modified By": firstNonEmptyString(
      getString(rawPackage, "modifiedBy"),
      getFieldValue(fieldMap, ["Modified By"]),
    ),
    "Modified Date": firstNonEmptyString(
      parseDateToIso(getString(rawPackage, "modifiedDT")),
      getFieldValue(fieldMap, ["Modified Date"]),
    ),
    "STRATUS.Package.Id": packageId,
    "STRATUS.Model.ShippingAddress.Attention": getFieldValue(fieldMap, [
      "STRATUS.Model.ShippingAddress.Attention",
    ]),
    "STRATUS.Model.ShippingAddress": getFieldValue(fieldMap, [
      "STRATUS.Model.ShippingAddress",
    ]),
    ...
    "Project Name": firstNonEmptyString(
      getFieldValue(fieldMap, ["Project Name"]),
      packageName,
    ),
    "STRATUS.Package.TrackingStatusId": trackingStatusId,
    "STRATUS.Package.FieldStartDT": firstNonEmptyString(
      parseDateToIso(getString(rawPackage, "startDT")),
      getFieldValue(fieldMap, ["STRATUS.Package.FieldStartDT"]),
    ),
    "STRATUS.Package.ModifiedDT": firstNonEmptyString(
      parseDateToIso(getString(rawPackage, "modifiedDT")),
      getFieldValue(fieldMap, ["STRATUS.Package.ModifiedDT"]),
    ),
    "STRATUS.Package.Container": getFieldValue(fieldMap, [
      "STRATUS.Package.Container",
    ]),
  };
...
export function getConfiguredPackageFieldValue(
  pkg: NormalizedStratusPackage,
  fieldName: string | null | undefined,
): string | null {
  const normalized = normalizeOptionalString(fieldName);
  if (!normalized) {
    return null;
  }
  ...
  const fieldMap = getRecord(pkg.rawPackage, "fieldNameToValueMap");
  return getFieldValue(fieldMap, getEquivalentFieldNames(normalized));
}
...
export function normalizeStratusProject(
  rawProject: Record<string, unknown>,
  ...
  return {
    ...
    status: firstNonEmptyString(
      getString(rawProject, "statusName"),
      toNullableString(rawProject.status),
    ),
...
function findFieldDefinitionMatch(
  ...
  const matches = fields.filter((field) => {
    ...
    const displayName =
      normalizeOptionalString(field.displayName)?.toLowerCase() ?? null;
  ...
  if (matches.length !== 1) {
    return {
      matchId:
        matches.length > 0 ? normalizeOptionalString(matches[0]?.id) : null,
