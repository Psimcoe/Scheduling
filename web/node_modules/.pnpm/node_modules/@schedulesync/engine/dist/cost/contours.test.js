...
describe('distributeWork', () => {
    it('distributes flat work evenly across periods', () => {
        const periods = distributeWork(WorkContour.Flat, 1000, '2025-01-06T00:00:00Z', '2025-01-17T00:00:00Z', 10);
    ...
    it('returns single period when span is zero', () => {
        const periods = distributeWork(WorkContour.Flat, 500, '2025-01-06T00:00:00Z', '2025-01-06T00:00:00Z');
        expect(periods).toHaveLength(1);
        expect(periods[0].work).toBe(500);
    });
    it('returns single period when totalWork is zero', () => {
        const periods = distributeWork(WorkContour.Bell, 0, '2025-01-06T00:00:00Z', '2025-01-17T00:00:00Z');
        expect(periods).toHaveLength(1);
        expect(periods[0].work).toBe(0);
    });
    it('total distributed work matches input for bell contour', () => {
        const periods = distributeWork(WorkContour.Bell, 2400, '2025-01-06T00:00:00Z', '2025-01-17T00:00:00Z', 10);
        const totalWork = periods.reduce((s, p) => s + p.work, 0);
        expect(totalWork).toBeCloseTo(2400, 1);
    });
    it('compresses when fewer periods than profile length', () => {
        const periods = distributeWork(WorkContour.FrontLoaded, 600, '2025-01-06T00:00:00Z', '2025-01-09T00:00:00Z', 3);
