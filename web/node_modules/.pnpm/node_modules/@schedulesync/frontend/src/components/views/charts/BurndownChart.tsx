...
const BurndownChart: React.FC<BurndownChartProps> = ({ tasks, width = 560, height = 280 }) => {
  ...
  return (
    <svg width={width} height={height} style={{ fontFamily: 'Segoe UI, sans-serif' }}>
      {/* Grid lines */}
      ...
      {/* X axis labels */}
      {data.labels.filter((_, i) => i % 3 === 0 || i === data.labels.length - 1).map((label, idx) => {
        const origIdx = data.labels.indexOf(label);
        return (
          <text key={idx} x={xScale(origIdx)} y={height - 8} textAnchor="middle" fontSize={9} fill="#666">
            {label}
          </text>
        );
      })}
