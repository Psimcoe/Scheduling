...
import { useProjectStore, useUIStore, type TaskRow, type FilterCriteria, type SortCriteria } from '../../stores';
...
function matchesCriterion(task: TaskRow, c: FilterCriteria): boolean {
  ...
  switch (c.operator) {
    ...
    case 'lt':
      return Number(raw) < Number(c.value);
    case 'between':
      return Number(raw) >= Number(c.value) && Number(raw) <= Number(c.value2);
    default:
      return true;
  }
}
