...
export default async function taskRoutes(app) {
    ...
    app.patch('/:taskId', async (req) => {
        ...
        if (body.finish !== undefined)
            data.finish = new Date(body.finish);
        ...
        if (body.percentComplete !== undefined)
            data.percentComplete = body.percentComplete;
        if (body.calendarId !== undefined)
            data.calendarId = body.calendarId;
        if (body.notes !== undefined)
            data.notes = body.notes;
        if (body.externalKey !== undefined)
            data.externalKey = body.externalKey;
    ...
    app.post('/batch', async (req) => {
        ...
        const results = await prisma.$transaction(body.updates.map((u) => {
            const data = {};
            const d = u.data;
            if (d.name !== undefined)
                data.name = d.name;
            if (d.durationMinutes !== undefined)
                data.durationMinutes = d.durationMinutes;
            if (d.start !== undefined)
                data.start = new Date(d.start);
            if (d.finish !== undefined)
                data.finish = new Date(d.finish);
            if (d.parentId !== undefined)
                data.parentId = d.parentId;
            if (d.type !== undefined)
                data.type = d.type;
            if (d.constraintType !== undefined)
                data.constraintType = d.constraintType;
            ...
            if (d.percentComplete !== undefined)
                data.percentComplete = d.percentComplete;
    ...
    app.delete('/:taskId', async (req, reply) => {
        ...
        // Remove baselines
        await prisma.baseline.deleteMany({ where: { taskId } });
        // Remove task
