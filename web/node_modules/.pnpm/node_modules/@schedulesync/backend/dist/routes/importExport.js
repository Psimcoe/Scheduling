...
import { CsvUpdateSource, JsonUpdateSource, StratusCsvUpdateSource, isStratusFormat, ImportOrchestrator, computeDiffs, validate as validateTask, DEFAULT_APPLY_OPTIONS, ConstraintType, TaskApplyStatus, } from '@schedulesync/engine';
...
export default async function importExportRoutes(app) {
    ...
    app.post('/import/preview', async (req, reply) => {
        ...
        const resolver = (update) => {
            const task = tasks.find((t) => (update.uniqueId != null && (parseInt(t.id, 10) === update.uniqueId || t.externalKey === String(update.uniqueId))) ||
    ...
    // ---------- Undo History List ----------
    app.get('/undo-history', async (req) => {
        const { projectId } = req.params;
...
async function buildProjectStore(projectId) {
    ...
    return {
        ...
        createTask(update, _options, _parentUniqueId) {
            return {
        ...
        findOrCreateSummaryTask(name, _parentUniqueId) {
            const existing = tasks.find((t) => t.type === 'summary' && t.name === name);
            if (existing)
        ...
        setDeadline(_uniqueId, _deadline) {
            // no-op for now
        },
        setCustomTextField(_uniqueId, _fieldName, _value) {
            // no-op for now
        },
    };
}
