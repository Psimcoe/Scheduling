...
export default async function dependencyRoutes(app) {
    ...
    app.patch('/:depId', async (req) => {
        ...
        if (body.lagMinutes !== undefined)
            data.lagMinutes = body.lagMinutes;
        const dep = await prisma.dependency.update({ where: { id: depId }, data });
        await recalculateProject(projectId);
        return dep;
    });
    // Delete dependency
    app.delete('/:depId', async (req, reply) => {
        const { projectId, depId } = req.params;
