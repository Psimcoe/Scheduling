 ...
 */
export async function captureUndo(projectId, description) {
 ...
 */
export async function undo(projectId) {
    ...
    if (!entry)
        return false;
    await restoreSnapshot(projectId, JSON.parse(entry.snapshotJson));
    await prisma.project.update({
        where: { id: projectId },
 ...
 */
export async function redo(projectId) {
    const project = await prisma.project.findUniqueOrThrow({
        where: { id: projectId },
        select: { undoPointer: true },
    });
    if (project.undoPointer === null)
        return false;
