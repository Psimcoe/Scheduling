...
export async function levelResources(projectId) {
    ...
    for (const a of assignments) {
        if (!a.resource || a.resource.type !== 'work')
            continue;
    ...
    for (const resource of resources) {
        ...
        if (!resAssignments || resAssignments.length <= 1)
            continue;
        ...
        const sorted = [...resAssignments].sort((a, b) => {
            ...
            if (slackA !== slackB)
                return slackA - slackB; // less slack = higher priority
        ...
        const projectStart = Math.min(...sorted.map((a) => a.task.start.getTime()));
        const scheduledSlots = [];
        let overAllocated = false;
        for (const assignment of sorted) {
            const taskStartDay = Math.floor((assignment.task.start.getTime() - projectStart) / (MINUTES_PER_DAY * 60000));
            const taskDurationDays = Math.max(1, Math.ceil(assignment.task.durationMinutes / MINUTES_PER_DAY));
            ...
            if (needsDelay) {
                ...
                if (found && candidate > taskStartDay) {
                    ...
                    const newStart = new Date(assignment.task.start.getTime() + delayMinutes * 60000);
                    const newFinish = new Date(assignment.task.finish.getTime() + delayMinutes * 60000);
 ...
 */
export async function clearLeveling(projectId) {
