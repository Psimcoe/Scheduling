 ...
 */
export function isStratusFormat(headerLine) {
    if (!headerLine)
        return false;
    const lower = headerLine.toLowerCase();
    return (lower.includes('prefab build start date') ||
        lower.includes('stratus.package.id') ||
        lower.includes('cost code number'));
}
export class StratusCsvUpdateSource {
    parse(content) {
        const updates = [];
        const errors = [];
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const getField = (name) => fields[colIndex.get(name) ?? -1]?.trim() ?? '';
            // Number (required)
            ...
            if (!packageId)
                packageId = getField('id');
            if (packageId)
                update.metadata['StratusPackageId'] = packageId;
            // Dates
            ...
            if (requiredStr) {
                ...
                if (d)
                    update.newDeadline = d.toISOString();
            }
...
//# sourceMappingURL=stratusCsvUpdateSource.js.map