 ...
 */
import { ConstraintType, ValidationSeverity, } from '../types.js';
const CONSTRAINT_ASAP = ConstraintType.ASAP;
/** Returns true if the update changes any date field. */
function hasDateChanges(update) {
    return (update.newStart != null ||
 ...
 */
export function validate(snapshot, update) {
    const warnings = [];
    ...
    if (snapshot.isManuallyScheduled && hasDateChanges(update)) {
        warnings.push({
            ...
            message: 'Task is manually scheduled. Date changes will be applied but will not trigger auto-scheduling.',
        });
    }
    // Rule 3: Active constraint + date changes + no override → error
    if (snapshot.constraintType !== CONSTRAINT_ASAP &&
        hasDateChanges(update) &&
        !update.allowConstraintOverride) {
        warnings.push({
            severity: ValidationSeverity.Error,
            message: 'Task has an active constraint. Set AllowConstraintOverride to true to proceed.',
        });
    }
 ...
 */
export function validateNew(update) {
    const warnings = [];
