...
export async function computeCosts(projectId: string) {
  ...
  // Persist computed costs back to DB in a transaction
...
export async function computeEarnedValue(projectId: string, statusDate?: string) {
  const projectData = await loadProjectData(projectId);
  const evSummary = computeProjectEV(projectData, statusDate);

  // Persist EV fields back to tasks in a transaction
  await prisma.$transaction(
    evSummary.taskResults.map((result) =>
      prisma.task.update({
