...
export default async function advancedRoutes(app: FastifyInstance) {
  ...
  app.post<{ Params: { projectId: string } }>(
    '/earned-value',
    async (req) => {
      const result = await computeEarnedValue(req.params.projectId);
      return result;
    },
  );
  ...
  app.post<{ Params: { projectId: string; taskId: string } }>(
    ...
    async (req) => {
      ...
      if (existing.length === 0) {
        ...
        if (segIdx === -1) return { error: 'Split date not within any segment' };
      ...
      for (let i = 0; i < tasks.length - 1; i++) {
        ...
        if (!existingPairs.has(key)) {
          await prisma.dependency.create({
            data: { projectId, fromTaskId: fromId, toTaskId: toId, type: 'FS', lagMinutes: 0 },
          });
          linked++;
        }
      }

      if (linked > 0) await recalculateProject(projectId);
      return { linked };
    },
  );
