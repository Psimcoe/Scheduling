...
export default async function importExportRoutes(app: FastifyInstance) {
  ...
  app.post<{ Params: { projectId: string } }>(
    ...
    async (req, reply) => {
      ...
      const resolver = (uniqueId: number): TaskSnapshot | null => {
        const task = tasks.find(
          (t) => parseInt(t.id, 10) === uniqueId || t.externalKey === String(uniqueId),
        );
        if (!task) return null;
        return {
          uniqueId,
    ...
    async (req, reply) => {
      ...
      for (const detail of result.details) {
        if (detail.status === 'Updated' && detail.diff) {
          ...
          const d = detail.diff;
          if (d.newStart) data.start = new Date(d.newStart);
          if (d.newFinish) data.finish = new Date(d.newFinish);
...
async function buildProjectStore(projectId: string): Promise<IProjectStore> {
  ...
  return {
    getSnapshot(uniqueId: number): TaskSnapshot | null {
      const task = tasks.find(
        (t) =>
          parseInt(t.id, 10) === uniqueId ||
    ...
    applyUpdate(uniqueId: number, update: TaskUpdate): boolean {
      // In-memory updates—actual persistence happens in the route handler
      return true;
    },

    createTask(update: TaskUpdate): number {
      // Won't be called in preview, but return a placeholder
      return -1;
    },

    setSummaryValue(uniqueId: number, fieldName: string, value: string): void {
      // no-op for preview
    },

    addToSelection(uniqueId: number): void {
      // no-op
    },

    openUndoTransaction(label: string): void {
      // handled at route level
    },

    closeUndoTransaction(): void {
      // handled at route level
    },

    getTaskByField(fieldName: string, value: string): number | null {
      const task = tasks.find(
        (t) =>
          (fieldName === 'Text30' && t.externalKey === value) ||
