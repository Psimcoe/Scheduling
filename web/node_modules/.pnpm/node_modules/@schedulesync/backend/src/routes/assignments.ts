...
export default async function assignmentRoutes(app: FastifyInstance) {
  ...
  // Update assignment
  app.patch<{ Params: { projectId: string; assignId: string } }>(
    '/:assignId',
    async (req) => {
      ...
      if (body.workMinutes !== undefined) data.workMinutes = body.workMinutes;
  ...
  // Delete assignment
  app.delete<{ Params: { projectId: string; assignId: string } }>(
    '/:assignId',
    async (req, reply) => {
      const { assignId } = req.params;
