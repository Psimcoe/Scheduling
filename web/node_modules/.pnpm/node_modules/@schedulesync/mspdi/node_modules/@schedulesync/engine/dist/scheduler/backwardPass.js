 ...
 */
export function backwardPass(sortedTaskIds, taskMap, depsByPredecessor, _earlyDates, calendarMap, defaultCalendar, projectFinish) {
    const lateDates = new Map();
    // Process in reverse topological order
    const reversed = [...sortedTaskIds].reverse();
    for (const taskId of reversed) {
        const task = taskMap.get(taskId);
        if (!task || task.type === TaskType.Summary)
            continue;
        const cal = (task.calendarId && calendarMap.get(task.calendarId)) || defaultCalendar;
        const addWM = (d, m) => addWorkingMinutes(d, m, cal);
        const subWM = (d, m) => subtractWorkingMinutes(d, m, cal);
        ...
        for (const dep of outgoingDeps) {
            ...
            if (!succLate)
                continue;
            const succCal = (taskMap.get(dep.toTaskId)?.calendarId &&
            ...
            switch (dep.type) {
                case DependencyType.FS:
                    ...
                    depDate =
                        dep.lagMinutes !== 0
                            ...
                            : succLate.lateStart;
                    if (depDate.isBefore(lateFinish))
                        lateFinish = depDate;
                    break;
                ...
                case DependencyType.SF:
                    ...
                    if (sfLF.isBefore(lateFinish))
                        lateFinish = sfLF;
                    break;
            }
        }
        // Compute late start from late finish
        let lateStart = task.type === TaskType.Milestone
            ? lateFinish
            : subWM(lateFinish, task.durationMinutes);
        // Apply constraints on backward pass
        const constrained = applyConstraintBackwardPass(task.constraintType, task.constraintDate, lateStart, lateFinish, task.durationMinutes, addWM, subWM);
...
//# sourceMappingURL=backwardPass.js.map